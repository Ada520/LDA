...
def load_texts(dataset_type='train', groups=None):
    ...
    """
    if groups == 'small':
        groups = ['comp.graphics', 'comp.os.ms-windows.misc']  # 仅用于小数据测试时用, #共有1368条数据
    elif groups == 'medium':
    ...
    return train_dataset_bunch.data#list,每一项代表一个文档
...
def preprocess_texts(texts, test_doc_id=1):
    ...
    #print texts[1]
    #print 'original texts[%d]: ' % test_doc_id, '\n',texts[test_doc_id]
    # split_texts = [t.lower().split() for t in texts]
    # print'split texts[%d]: #%d' % (test_doc_id, len(split_texts)), '\n',split_texts[test_doc_id]
    ...
    for t in texts:
        while '' in t:
            t.remove('')
    #print'texts[%d] lower & split(seps= %s) & delete None: #%d' % (test_doc_id, SEPS, len(texts[test_doc_id])),  '\n',texts[test_doc_id]


    # nltk.download()   #then choose the corpus.stopwords
    stopwords = set(nltk.corpus.stopwords.words('english'))  # #共127个
    stopwords.update(['from', 'subject', 'writes'])  # #共130个
    ...
    #print len(texts)#1368
    too_common_words=[]
    for t in texts:
        for w in t:
            if word_usage[w] > COMMON_LINE and w not in too_common_words:
                too_common_words.append(w)
...
def save_corpus_dict(dict, corpus, dictDir='./id_word.dict', corpusDir='./corpus.mm'):
    dict.save(dictDir)
    print ('dict saved into %s successfully ...' % dictDir)
    corpora.MmCorpus.serialize(corpusDir, corpus)
    print ('corpus saved into %s successfully ...' % corpusDir)
...
def load_ldamodel(modelDir='./lda.pkl'):
    ...
    print( 'ldamodel load from %s successfully ...' % modelDir)
    return model


def load_corpus_dict(dictDir='./id_word.dict', corpusDir='./corpus.mm'):
    dict = corpora.Dictionary.load(fname=dictDir)
    print( 'dict load from %s successfully ...' % dictDir)
    # dict = corpora.Dictionary.load_from_text('./id_word.txt')
    corpus = corpora.MmCorpus(corpusDir)  # corpora.mmcorpus.MmCorpus
    print( 'corpus load from %s successfully ...' % corpusDir, )
    return dict, corpus
...
def closest_texts(corpus, model, num_topics, test_doc_id=1, topn=5):
    ...
    #     print 'closest doc[%d]' % closest_doc_id, '\n', original_texts[closest_doc_id])
...
def test():
    ...
    print( 'model.log_perplexity for test_texts', evaluate_model(model))
